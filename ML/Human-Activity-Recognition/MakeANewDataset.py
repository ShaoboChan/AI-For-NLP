...
def generrate_signal(dir):
    ...
    for i in range(4,7):
        ...
        tem=norm(tem)
        gyroysig.append(tem)
    x = np.linspace(0, 1, len(df[1][:]))
    return  accelsig,gyroysig,x
    #
...
#348
    ...
    return FEATURE
def spl(dirraw,dirtrain,dirtest):
    df = pd.read_csv(dirraw, header=None, sep=',')
    ...
    l=len(df1[1])
    cutline=l*(7/10)
    train=df1.iloc[:cutline]
    test=df1.iloc[cutline:]
    train.to_csv(dirtrain)
    test.to_csv(dirtest)
...
if __name__=='__main__':
    ...
    windowsize = 2 * fss
    stride = fss
    accsig, gyrosig,ax= generrate_signal('mydataset/walk323_1.csv')
    gx=ax
    accsig_idx, gyrosig_idx = [[]] * 3, [[]] * 3
    all=[]
    idx=0
    for i in range(2*fss, len(accsig[0]) - 2* fss, fss):
        for j in range(3):
        ...
        feature_idx = ALLFeature(fss, accsig_idx, gyrosig_idx,  ax)
        # feature_idx.append(0)
        # all.append(feature_idx)
        print(idx,feature_idx)
        idx+=1
        import csv
        with open("walk323.csv", "a", newline='') as csvfile:
            ...
            csvfile.close()
    # allnp=np.array(all)
    # dftrain=pd.DataFrame(allnp)
    # dftrain.to_csv("walk11111.csv", header=None, index=True)
